...
def count_activation_size(
    net, input_size=(1, 3, 224, 224), require_backward=False, activation_bits=32
):
        ...
        if len(list(m.children())) == 0:
            ...
            def new_forward(_module):
                def lambda_forward(_x):
                    current_act_size = (
                        _module.tmp_activations + memory_info_dict["residual_size"]
                    )
...
def profile_memory_cost(
    ...
    activation_size = count_activation_size(
...
class Profiler(object):
    ...
    def profile(self):
        ...
        with open(
            self.base_dir / "designs" / self.design / "mapper/mapper.yaml", "r"
        ) as fid:
            mapper_dict = yaml.safe_load(fid)
            for layer_id, info in layer_info.items():
                layer_info[layer_id]["mapper_timeout"] = mapper_dict["mapper"][
                    "timeout"
                ]
                layer_info[layer_id]["mapper_algo"] = mapper_dict["mapper"]["algorithm"]
                layer_info[layer_id]["mapper_victory_condition"] = mapper_dict[
                    "mapper"
                ]["victory-condition"]
                layer_info[layer_id]["mapper_max_permutations"] = mapper_dict["mapper"][
        ...
        for layer_id, info in layer_info.items():
            for profiled_name, profiled_info in self.profiled_lib.items():
                if (
                    ...
                    and info["mapper_victory_condition"]
                    == profiled_info["mapper_victory_condition"]
                    and info["mapper_max_permutations"]
                    == profiled_info["mapper_max_permutations"]
                ):
        ...
        def get_cmd(layer_id):
            cwd = (
                f"{self.base_dir/'designs'/self.design/self.sub_dir/f'layer{layer_id}'}"
            )
            if "M" in layer_info[layer_id]["layer_dict"]["problem"]["instance"]:
                constraint_pth = (
                    self.base_dir / "designs" / self.design / "constraints/*.yaml"
                )
            else:
                # depthwise
                constraint_pth = (
                    self.base_dir / "designs" / self.design / "constraints_dw/*.yaml"
                )

            timeloopcmd = (
                f"timeloop-mapper "
                f"{self.base_dir/'designs'/self.design/'arch'/self.design}.yaml "
                f"{self.base_dir/'designs'/self.design/'arch/components/*.yaml'} "
                f"{self.base_dir/'designs'/self.design/'mapper/mapper.yaml'} "
                f"{constraint_pth} "
        ...
        for layer_id in layer_info.keys():
            ...
            if layer_name not in self.profiled_lib.keys():
                info = {
                    ...
                    "mapper_victory_condition": layer_info[layer_id][
                        "mapper_victory_condition"
                    ],
                    "mapper_max_permutations": layer_info[layer_id][
        ...
        overall["num_params"] = sum(
            p.numel() for p in self.model.parameters() if p.requires_grad
        )
        overall["macs"] = profile_macs(
            self.model, torch.randn([1] + list(self.input_size))
        )
        overall["activation_size"] = count_activation_size(
            self.model, [1] + list(self.input_size)
        )
...
def test():
    ...
    profiler = Profiler(
        top_dir="workloads",
        sub_dir="alexnet",
        design="simple_weight_stationary",
