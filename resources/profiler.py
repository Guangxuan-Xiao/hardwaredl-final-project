...
class Profiler(object):
    ...
    def profile(self):
        ...
        for layer_id in layer_info.keys():
            os.makedirs(
                self.base_dir / self.timeloop_dir / self.sub_dir / f"layer{layer_id}",
                exist_ok=True,
            )
        ...
        def get_cmd(layer_id):
            ...
            if "M" in layer_info[layer_id]["layer_dict"]["problem"]["instance"]:
                constraint_pth = (
                    self.base_dir / self.timeloop_dir / "constraints/*.yaml"
                )
            else:
                # depthwise
                constraint_pth = (
                    self.base_dir / self.timeloop_dir / "constraints_dw/*.yaml"
                )
